...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-black navbar-dark" id="navmain">
    ...
    <ul class="navbar-nav">
      ...
      <li class="nav-item d-none d-sm-inline-block">
        <a href="#" data-toggle="modal" data-target="#aboutModal" class="nav-link">About</a>
      </li>
      <li class="nav-item d-none d-sm-inline-block">
        <a href="#" data-toggle="modal" data-target="#reviewsModal" class="nav-link">Reviews</a>
      </li>
    </ul>
...
<div style="overflow-y:hidden" class="modal fade" id="reviewsModal" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" id="revdia">
    <div id="mc" class="modal-content">
      @if(\Session::has('successrev'))
<div id="success-alert11" class="alert alert-success alert-block" style="background-color: green; text-align:center;  margin-bottom: 0px;"><strong>{{\Session::get('successrev')}}</strong></div>
@endif
      <div class="modal-header">
        <li class="nav-item d-none d-sm-inline-block">REVIEWS</li>
      </div>
      <div class="modal-body" id="revbody">
        @foreach($reviews as $review)
        <div class="container" id="rw">
          <div class="container" ><span style="margin-bottom:2rem; color:yellow;width:30px;height:30px; font-size:30px" class="fa fa-star checked"><p style="color:black; margin-top:-1.5rem; margin-left:13px; font-size:18px">{{$review['rating']}}</p></div>
          <p style="white-space: pre-line; word-wrap: break-word;">{{$review['review']}}</p>
        </div>
        @endforeach
      </div>
      <form id="formpostreview" action="{{ action('ReviewsController@postreview') }}" method="POST" enctype="multipart/form-data">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
      <div class="modal-footer">
        <div class="wrapper" id="pc2">
        <div  id="b3" class="container">
          <div id="inp" class="form-group">
            <textarea style="overlay-y: scroll; height:120px; width: 550px; margin-bottom:-1.2rem" class="form-control description_c" id="review" name="review" placeholder="Write your review here"></textarea>
            @if ($errors->has('review')) <p style="margin-top:1.1rem; margin-bottom:-1.1rem"><small class="help-block" style= " color:red;">{{ $errors->first('review') }}</small></p> @endif
          </div>
        </div>
        <div id="inp" class="container" style="border-left: 1px solid white">
          <div class="form-group">
          <label>Rate us:</label>
          <fieldset style="mrgin-left:-1rem" class="rating">
        ...
        @if ($errors->has('rating')) <p style="margin-bottom:-1rem; margin-top:-1rem"><small class="help-block" style= " color:red;">{{ $errors->first('rating') }}</small></p> @endif
          </div>
          <div class="form-group" style="margin-left:-1rem margin-top:-2.5rem">
        <button type="submit" id="revb" class="btn btn-bbb">Post</button>
        <button type="button" id="revb" class="btn btn-bbb2" data-dismiss="modal">Close</button>
          </div>
        </div>
      </div>
      </div>
  ...
  <aside id="aside" class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="{{ url('/') }}" class="brand-link" style="font-size: 2vw">
      <img src="{{ asset('dist/img/AdminLTELogo.png') }}" alt="AdminLTE Logo" class="brand-image img-circle elevation-3"
           style="opacity: .8">
      <span class="brand-text font-weight-light">VrhovacMedia</span>
    </a>   
          ...
          @if(Auth::check())
            ...
            <li class="nav-item d-none d-sm-inline-block">
              <button id="addproduct" type="button" class="btn btn-bbb" data-toggle="modal" data-target="#myModal" >
                ...
                <a class="nav-item" style="margin-bottom: 1vw; color:white; border-bottom:1px white solid" href="#infoModal" data-toggle="modal"><i class="fas fa-info-circle"></i> Shipping policy information</a>
...
@if ($errors->has('name'))
    ...
    <script>
        ...
        @elseif ($errors->has('rating') || $errors->has('review'))
        <script>
            $( document ).ready(function() {
...
@elseif(\Session::has('successrev'))
<script>
  $( document ).ready(function() {
      $('#reviewsModal').modal('show');
      $("#success-alert11").fadeTo(2000, 500).slideUp(500, function(){
      $("#success-alert11").slideUp(500);
});
  });
</script>
